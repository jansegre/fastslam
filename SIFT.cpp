...
void SIFT::ExibeDescritor(){

	// KeyPoint ==> Descritor com 128 Valores
	double FV[128];
	
	//printf("Descritor dos KeyPoints\n ");
	for(int i=0;i<m_numKeypoints;i++)
	{
		printf("( %f , %f  )\n ", m_keyDescs.at(i).xi, m_keyDescs.at(i).yi);
		for(int j=0;j<FVSIZE;j++)
		{
			FV[j]=m_keyDescs.at(i).fv.at(j);
			printf("%f, ", FV[j]);
...
int SIFT::GetDescritor(double *BuffF){

	for(int i=0;i<m_numKeypoints;i++)
	{
		for(int j=0;j<FVSIZE;j++)
		{
			*BuffF=m_keyDescs.at(i).fv.at(j);
			 BuffF=BuffF+1;
		}
	}
